...
type ColumnType = ColumnDef<EmergencyInterface, unknown>;

interface EmergencyListPageProps {
...
export function EmergencyListPage(props: EmergencyListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getEmergencies({
        relations: ['service_request'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['emergency_type.contains', 'status.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<EmergencyInterface>>(
    () => `/emergencies?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: EmergencyInterface) => {
    if (hasAccess('emergency', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/emergencies/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'emergency_type', header: 'Emergency Type', accessorKey: 'emergency_type' },
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Link
              ...
              href={`/service-requests/view/${record.service_request?.id}`}
            >
              {record.service_request?.service_type}
            </Link>
          ),
        }
      : null,
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/emergencies/view/${record.id}`} passHref legacyBehavior>
                <Button
                  onClick={(e) => e.stopPropagation()}
                  mr={2}
                  padding="0rem 8px"
