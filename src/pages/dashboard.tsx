...
import { getEmergencies } from 'apiSdk/emergencies';
...
import { ServiceHistoryListPage as ShowCaseList } from 'pages/service-histories';

function DashboardPage() {
  const { session } = useSession();

  const emergencyStat = useSWR<PaginatedInterface<EmergencyInterface>>(
    () => `/emergencies`,
    () => getEmergencies({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Emergencies',
      result: emergencyStat?.data?.totalCount,
      isLoading: emergencyStat.isLoading,
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Service Histories
                </Text>
                <NextLink href={`service-histories`} passHref legacyBehavior>
                  <Button
